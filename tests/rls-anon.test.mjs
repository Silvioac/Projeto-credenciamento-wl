...
// Rodar: npm run test:rls   (usa NEXT_PUBLIC_SUPABASE_URL / NEXT_PUBLIC_SUPABASE_ANON_KEY do .env.local)
...
test("anônimo NÃO consegue ler o próprio registro", async () => {
  const { data, error } = await anon.from("inscritos").select("*").eq("codigo", codigo);
  assert.equal(error, null, error?.message);
  assert.equal(data.length, 0, "RLS deveria esconder a linha");
});

test("anônimo NÃO consegue listar nem contar a base", async () => {
  const { data, count, error } = await anon.from("inscritos").select("codigo", { count: "exact" }).limit(5);
  assert.equal(error, null, error?.message);
...
test("anônimo NÃO consegue fazer check-in (update)", async () => {
  const { data, error } = await anon.from("inscritos").update({ presente: true }).eq("codigo", codigo).select();
  assert.equal(error, null, error?.message);
  assert.equal(data.length, 0, "nenhuma linha deveria ser atualizada");
});

test("anônimo NÃO consegue apagar", async () => {
  const { data, error } = await anon.from("inscritos").delete().eq("codigo", codigo).select();
  assert.equal(error, null, error?.message);
  assert.equal(data.length, 0, "nenhuma linha deveria ser apagada");
});

test("views do painel não vazam dados para anônimo", async () => {
  const { data, error } = await anon.from("painel_resumo").select("*");
  assert.equal(error, null, error?.message);
...
after(async () => {
  // Limpeza: só é possível com um token de gerenciamento (nunca com a chave pública — e é isso que o teste prova).
  const token = env.SUPABASE_ACCESS_TOKEN;
  const ref = URL_SUPABASE.match(/https:\/\/([a-z0-9]+)\.supabase\.co/)?.[1];
  if (!token || !ref) {
    console.log(`\nRegistro de teste ${codigo} ficou na base (apague pelo painel ou defina SUPABASE_ACCESS_TOKEN para limpeza automática).`);
    return;
  }
  await fetch(`https://api.supabase.com/v1/projects/${ref}/database/query`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ query: `delete from public.inscritos where codigo = '${codigo}'` }),
  });
